...
class TestUserRole:
    ...
    def initial_role(self, app, create_super_user):
        with app.app_context():
            user_access = UserAccess()
            role_record = Role(**initial_role_data)
            db.session.add(role_record)
            db.session.commit()
            user_access.assign_role(create_super_user, role_record.id)
    ...
    def test_get_all_user_roles(self, client, login_super_user, initial_role):
    ...
    def test_create_user_role(self, app, client, login_super_user,
                              role_for_user, create_super_user):
        ...
        with app.app_context():
            user = User.query.filter_by(id=str(create_super_user)).first()
            roles = user.roles
            assert any([str(role.id) == role_for_user['role_id']
                        for role in roles])

    def test_delete_role_from_user(self, app, client, login_super_user, initial_role, create_user):
